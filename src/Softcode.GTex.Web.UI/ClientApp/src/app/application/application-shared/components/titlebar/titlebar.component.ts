...
export class TitlebarComponent implements AfterViewInit {
    ...
    constructor(private titleService: Title) {
        ...
        this.uniqueId = "tb_" + Math.floor(Math.random() * Math.floor(10000)) + "_" + Math.floor(Math.random() * Math.floor(100000));
    }
    ...
    addDetailPageToolbarItem(externalItem): void {
        ...
        titleItem.template = () => {
            return '<div  style=\'font-size:16px; color:#ffffff;\' class=\'toolbar-label ' + this.uniqueId + '\'>' + this.title + '</div>';
        };
        ...
        saveNnewItem.visible = !this.hideSaveAndNew;
    ...
    initializeToolbar(title: string, item: any, toolbarType: ToolbarType) {
    ...
    setToolbarTitle(title: string): void {
        //console.log('title bar title change call' + title);
        this.title = title;
        $('.' + this.uniqueId).text(title);
    ...
    setbrowserTitle(title: string) {
        ...
        if (title.indexOf('New') > 0) {
            ...
            let splitted = title.split(":", 2)
            if (splitted.length > 1) {
                this.titleService.setTitle(splitted[1]);
            }
            else {
                this.titleService.setTitle(title);
            }
