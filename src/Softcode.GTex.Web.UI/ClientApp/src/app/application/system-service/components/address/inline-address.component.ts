...
export class InlineAddressComponent implements OnInit, AfterContentChecked {
    ...
    @Input('BusinessProfileIds') businessProfileIds: any = [];
    ...
    ngAfterContentChecked(): void {
        ...
        //if (this.businessProfileId == 0 && this.businessProfileIds.length > 0) {
        //    this.businessProfileId = this.businessProfileIds[0];
        //}

        if (this.uniqueEntityId != null && this.uniqueEntityId != undefined && this.uniqueEntityId !='00000000-0000-0000-0000-000000000000' && this.entityId > 0) {
    ...
    LoadAddressTypeSelectBoxByBpId(businessProfileId:number): void {
        this.addressService.getAddressTypeDataSource(businessProfileId).toPromise().then((response: any) => {

            this.addressTypeSelectItemsDataSource = response.result;
        });
    }

    LoadAddressTypeSelectBoxByBpIds(businessProfileId: number[]): void {
        if (businessProfileId.length > 0) {
            this.addressService.getAddressTypeDataSourceByBpIds(businessProfileId).toPromise().then((response: any) => {

                this.addressTypeSelectItemsDataSource = response.result;
            });
        }
        else {
            this.addressTypeSelectItemsDataSource = null;
        }
    }

    private loadAddressInitData(): void {

        if (!this.isInitDataLoaded) {

          
            this.addressService.getInitAddressData(this.uniqueEntityId).toPromise().then((response: any) => {
    ...
    loadAddressInfo(): any {
        if (this.recordId >= 0) {
            this.addressService.getRecordInfoById(this.recordId, this.uniqueEntityId, this.entityId, this.businessProfileId).toPromise().then((response: any) => {
                ...
                if (this.recordId == 0) {
                    this.formDataSource.addressTypeId = null;
                    this.formDataSource.countryId = null;
                }
    ...
    validateAndSave(): boolean {
        ...
        if (this.showDetail) {
            ...
            this.saveEntity(DetailPageAction.Save);
            return true;
        }
        else {
            return true;
