...
export class BusinessProfileGeneralInformationComponent {
  @ViewChild('itmForm') itmForm: NgForm;
  ...
  @ViewChild('domainNameValidation')
  private domainNameValidation: DxValidatorComponent;
  ...
  attachValidationToControl() { 
    ...
    //validation
    this.domainNameValidation.validationRules = [{ type: 'required', message: 'Domain Name is required.' }];
    this.companyNameValidation.validationRules = [{ type: 'required', message: 'Name is required.' }];
    this.emailValidation.validationRules = [{ type: 'pattern', pattern: this.emailPattern, message: 'Email is invalid' }];
  }
...
* */
  init(): void {
    ...
    this.businessProfileService.getBusinessProfile(this.businessProfileId).toPromise().then((response: any) => {
      ...
      this.titlebar.initializeToolbar("Business Profile", null, ToolbarType.DetailTabPage);
      //this.businessProfileDataSource.companyName, null, this.toolbarType);
    });
  }
  ...
  redirectToListPage(action: DetailPageAction): void {
    ...
    if (action == DetailPageAction.Close || action == DetailPageAction.SaveAndClose) {
      ...
      this.businessProfileDataSource = JSON.parse(JSON.stringify(this.emptyBusinessProfileDataSource));
  ...
  validateAndSave(action: DetailPageAction): void {
    console.log(this.itmForm.dirty);
